...
class Cluster {
    constructor() {
        //@ts-ignore
        consoles.log = (logger, ...str) => process.send({ op: "log", msg: str, logger });
        //@ts-ignore
    ...
    async connect() {
        ...
        if (this.whatToLog.includes('cluster_start'))
            consoles.log('cluster_start', `Connecting with ${this.shards} shard(s)`);
        ...
        bot.on("connect", (id) => {
            ...
            if (this.whatToLog.includes('shard_connect'))
                consoles.log('shard_connect', `Shard ${id} connected!`);
        });
        bot.on("shardDisconnect", (err, id) => {
            //@ts-ignore
            if (!this.shutdown)
                if (this.whatToLog.includes('shard_disconnect'))
                    consoles.log('shard_disconnect', `Shard ${id} disconnected with error: ${util_1.inspect(err)}`);
        });
        bot.on("shardReady", (id) => {
            //@ts-ignore
            if (this.whatToLog.includes('shard_ready'))
                consoles.log('shard_ready', `Shard ${id} is ready!`);
        });
        bot.on("shardResume", (id) => {
            //@ts-ignore
            if (this.whatToLog.includes('shard_resume'))
                consoles.log('shard_resume', `Shard ${id} has resumed!`);
        });
        ...
        bot.on("ready", (id) => {
            ...
            if (this.whatToLog.includes('cluster_ready'))
                consoles.log('cluster_ready', `Shards ${this.firstShardID} - ${this.lastShardID} are ready!`);
        });
        bot.once("ready", () => {
            this.loadCode(App);
            //@ts-ignore
